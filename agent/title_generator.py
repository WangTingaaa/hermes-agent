...
def _title_language() -> str:
    """Use the title override, then the display locale, then match the user."""
    try:
        title_language = str(_title_config().get("language", "")).strip()
        if title_language:
        ...
        display_language = str(
            ((load_config_readonly() or {}).get("display") or {}).get("language", "")
        ).strip()
