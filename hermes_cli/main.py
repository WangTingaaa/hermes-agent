...
Usage:
    ...
    hermes uninstall           Uninstall Mira Agent
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Mira Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_uninstall(args):
    """Uninstall Mira Agent (or just the Chat GUI with --gui)."""
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Mira Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Mira Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    ...
                    print("  running Mira desktop window and retry.")
    ...
    if source_mode:
        print("→ Launching Mira Desktop from source build...")
    ...
    print(f"→ Launching packaged Mira Desktop: {' '.join(launch_command)}")
...
def _update_via_zip(args):
    """Update Mira Agent by downloading a ZIP archive.
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Mira Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def cmd_update(args):
    """Update Mira Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Mira Agent")
        return
...
def cmd_acp(args):
    """Launch Mira Agent as an ACP server."""
    try:
