...
def _print_side_result_panel(cli, *, header_lines, body, title_suffix, empty_note, console=None) -> None:
    ...
    try:
        ...
        label, _resp_color, _resp_text = "⚕ Mira", "#CD7F32", "#FFF8DC"
...
class CLICommandsMixin:
    ...
    def _handle_update_command(self) -> bool:
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Mira Agent')}")
            return False
        # prompt_toolkit-native modal: renders above the composer, no raw input() races.
        choices = [("once", "Update Now", "exit the current session and update Mira Agent"),
                   ("cancel", "Cancel", "keep the current session")]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Mira Agent",
