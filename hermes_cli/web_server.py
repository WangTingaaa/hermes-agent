...
app = FastAPI(title="Mira Agent", version=__version__, lifespan=_lifespan)
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Mira Agent").strip() or "Mira Agent"
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Mira Agent exposes a
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  Mira backend listening on {host}:{actual_port}", flush=True)
            else:
                print(f"  Mira Web UI → http://{host}:{actual_port}")
