...
def _custom_endpoint_response(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(providers, dict):
        for provider_id, raw_entry in providers.items():
            ...
            endpoints.append(_endpoint_row(
                endpoint_id, endpoint_name, base_url,
                ...
                raw_entry,
                current_provider.lower() in custom_provider_aliases(endpoint_name, endpoint_id),
                "providers",
            ))
    ...
    if current_custom and current_base_url and not any(e["is_current"] for e in endpoints):
        ...
        key_entry = display_entry if _api_key_display(display_entry)[0] else model_cfg
        endpoints.insert(0, _endpoint_row(
            _custom_endpoint_id(endpoint_identity),
            str(display_entry.get("name") or "Custom"),
            current_base_url,
            str(display_entry.get("model") or current_model),
            endpoint_models,
            display_entry.get("context_length", model_cfg.get("context_length")),
            bool(display_entry.get("discover_models", True)),
            key_entry,
            True,
            "direct-config",
        ))
