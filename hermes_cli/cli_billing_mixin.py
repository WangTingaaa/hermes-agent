...
_CHARGE_ERROR_COPY = {
    ...
    "cli_billing_disabled": "  Remote spending is off for this account — a billing admin can turn it on from the portal's Mira Agent page.",
...
_KILLSWITCH_REASON_OVERVIEW = "A billing admin can turn it on from the portal's Mira Agent page to add funds here."
_KILLSWITCH_REASON_BUY = "A billing admin can turn it on from the portal's Mira Agent page before adding funds."
...
class CLIBillingMixin:
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's Mira Agent page, then run /topup again.")
