"""Interactive setup wizard for Mira Agent (config lives in ~/.hermes/).
...
def _run_setup_section(config: dict, section: str) -> None:
    ...
    if entry is None:
        ...
        return
    label, func = entry
    _print_banner(f"│     ⚕ Mira Setup — {label:<34s} │")
...
def _run_setup_wizard_impl(args):
    ...
    _print_banner("│             ⚕ Mira Agent Setup Wizard                  │",
                  "├─────────────────────────────────────────────────────────┤",
                  "│  Let's configure your Mira Agent installation.       │",
