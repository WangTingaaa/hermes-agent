...
// ACTIVE_HERMES_ROOT — the canonical mutable Mira install. Same path
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  ...
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Mira backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Mira backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Mira backend PID ${child.pid}; ` +
  ...
  try {
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Mira backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Mira — this window will close. Don’t reopen Mira yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
async function resolveHermesBackend(backendArgs) {
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Mira runtime at ${ACTIVE_HERMES_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
...
async function runEnsureRuntime(backend: any, assertStillOwned: () => void): Promise<any> {
  ...
  if (!isHermesSourceRoot(ACTIVE_HERMES_ROOT)) {
    throw new Error(missingInstallPartMessage(`Mira source files are missing or incomplete at ${ACTIVE_HERMES_ROOT}`))
  }
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      "Mira needs a helper called Git for Windows, which isn't installed. " +
        'Choose Repair install to add it automatically, or install it yourself from git-scm.com and reopen Mira.'
    )
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Mira Cloud agent…' : 'Sign in to Hermes gateway',
...
// --- Mira Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Mira Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
// Discover the hosted (Mira Cloud) agents the signed-in user can see. Calls
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Mira Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Mira Cloud org tag on cloud-mode entries so Settings can
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  return {
    ...
    remoteUrl,
    // The persisted Mira Cloud org (slug/id) for a cloud connection, or '' for
...
// `org` (optional) is the Mira Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Mira Cloud instance, NOT a user-owned remote gateway,
...
async function runPoolBackendStart(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  child.once('error', error => {
    ...
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Mira backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Mira backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Mira backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
async function runHermesStart() {
  ...
  const connectionPromise = (async () => {
    ...
    hermesProcess.once('exit', (code, signal) => {
      ...
      if (!backendReady) {
        const message = `Mira backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Mira backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }
...
// --- Mira Cloud (cloud-auto-discovery Phase 3) ---
