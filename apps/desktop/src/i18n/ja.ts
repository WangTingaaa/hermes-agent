...
export const ja = defineLocale({
  ...
  settings: {
    ...
    notifications: {
      ...
      focusedHint: '完了通知は Mira Agent がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
    ...
    gateway: {
      ...
      intro:
        'Mira Agent Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Mira Agent バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
  ...
  updates: {
    ...
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると Mira Agent が自動的に再接続します。',
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'Mira Agent 推論ゲートウェイのステータス',
      agents: 'エージェント',
